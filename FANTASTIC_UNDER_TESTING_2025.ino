...
#define AUTO_START_DELAY 30000 // 30 seconds delay
...
void startPumpWithDelay(); // New function for delayed start
...
void setup() {
    ...
    checkAutoModeOnStartup(); // Checks auto mode and sets the flag
...
void processCommand() {
    ...
    if (isRegisteredNumber(GSM_Nr)) {
        if (GSM_Msg == "#1") {
            ...
            status += "PUMP: " + String(pumpState ? "ON" : "OFF") + "\n";
...
void checkAutoModeOnStartup() {
    ...
    if (autoMode) {
        startPumpWithDelay(); // Start the pump with a delay if autoMode is ON
    } else {
        Serial.println("Auto mode is OFF.");
    }
}

void startPumpWithDelay() {
    Serial.println("Auto mode is ON.  Waiting for delay...");
    delay(AUTO_START_DELAY); // Wait for 30 seconds

    digitalWrite(Relay1, LOW); // Turn ON Relay1
    delay(3000);           // Keep it ON for a moment (as per your original logic)
    digitalWrite(Relay1, HIGH); // Turn OFF Relay1
    pumpState = true;      // Set pump state to ON
    Serial.println("Pump activated after delay.");
    sendSMS(masterNumber, "PUMP ON (Auto)"); // Notify that the pump turned on automatically
